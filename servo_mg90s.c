...
{
    servo->pwm = pwm;
    fallbacks(pwm_set_dt, servo->pwm, SERVO_MG90S_FREQ, SERVO_MG90S_POS_0);
...
{
    fallbacks(pwm_set_dt, servo->pwm, SERVO_MG90S_FREQ, direction);
    k_timer_start(&servo->timer, SERVO_MG90S_TIMER_TICKS(ticks), K_NO_WAIT);
...
{
    return pwm_set_dt(servo->pwm, SERVO_MG90S_FREQ, direction);
}

int servo_mg90s_rotate_stop(servo_mg90s_t *servo, int32_t direction)
{
    return pwm_set_dt(servo->pwm, SERVO_MG90S_FREQ, SERVO_MG90S_POS_0);
}